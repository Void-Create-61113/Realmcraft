...
function move(dt){let speed=5,dx=(keys.d?1:0)-(keys.a?1:0),dz=(keys.s?1:0)-(keys.w?1:0);const len=Math.hypot(dx,dz)||1;dx/=len;dz/=len;const sin=Math.sin(yaw),cos=Math.cos(yaw),vx=(dx*cos-dz*sin)*speed*dt,vz=(dx*sin+dz*cos)*speed*dt;let nx=camera.position.x+vx,nz=camera.position.z+vz;const feet=camera.position.y-1.6;if(canStand(nx,nz,Math.floor(feet)))camera.position.x=nx;if(canStand(camera.position.x,nz,Math.floor(feet)))camera.position.z=nz}
let vy=0,onGround=false;
function physics(dt){const feet=camera.position.y-1.6,below=get(Math.floor(camera.position.x),Math.floor(feet-.05),Math.floor(camera.position.z));if(below){camera.position.y=Math.floor(feet)+2.6;vy=0;onGround=true}else{vy-=18*dt;camera.position.y+=vy*dt;onGround=false}if(keys[' ']&&onGround){vy=7;keys[' ']=false}}
